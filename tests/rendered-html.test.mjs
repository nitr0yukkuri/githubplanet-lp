...
test("keeps the current renderer and smooth-scroll architecture wired", async () => {
  const [
    page,
    experience,
    hero,
    ...
    readFile(new URL("../components/landing/FinalCtaSection.tsx", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(scene, /snapshot\.phase !== "content"/);
  assert.match(scene, /planetAssetAvailable/);
  ...
  assert.doesNotMatch(readout, /visually-hidden/);
